...
const beforePhoto = await page.locator('#chat .msg.ai').count();
await page.locator('#photoInput').setInputFiles({ name: 'smoke.png', mimeType: 'image/png', buffer: png });
await page.waitForFunction(n => document.querySelectorAll('#chat .msg.ai').length > n && [...document.querySelectorAll('#chat .msg.ai')].some(x => !x.textContent.includes('⏳') && !x.textContent.includes('Готов. Напиши') && x.textContent.trim().length > 20), beforePhoto, { timeout: 120000 });
