...
function dispatch({ prompt = '', hasImage = false, hasFile = false, hasVideo = false } = {}) {
  ...
  // Проверка и диагностика должны иметь приоритет над обычным интернет-поиском.
  if (/(проверь|проверить|диагност|ошибк|ошибка|проблем|не работает|сломал|сломалось|исправь|исправить|почини|починить|почему не работает)/u.test(text)) return 'verification';
