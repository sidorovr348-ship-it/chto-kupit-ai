...
function dispatch({ prompt = '', hasImage = false, hasFile = false, hasVideo = false } = {}) {
  ...
  // Запросы вида «найди iPhone», «где дешевле», «есть в наличии»
  // должны попадать именно в модуль товаров.
  if (/(купи|купить|товар|цена|магазин|ozon|озон|wildberries|wb|яндекс.?маркет|dns|м\.видео|ситилинк|в наличии|дешевле|где купить|стоимость)/u.test(text)) return 'shopping';
