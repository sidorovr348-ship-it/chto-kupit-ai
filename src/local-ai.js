...
const OLLAMA_TIMEOUT_MS = Number(process.env.OLLAMA_TIMEOUT_MS || 42000);
...
async function callVision(prompt, images, options = {}) {
  ...
  const controller = new AbortController(); const timer = setTimeout(() => controller.abort(), options.timeoutMs || 30000);
  try {
    const body = { model: options.model || OLLAMA_MODEL, messages: [{ role: 'system', content: SYSTEM_PROMPT }, { role: 'user', content, images: encoded }], stream: false, think: false, options: { temperature: 0.4, num_predict: Number(options.numPredict || 300) } };
    const response = await fetch(`${OLLAMA_BASE_URL}/api/chat`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body), signal: controller.signal });
    const text = await response.text(); let data = {}; try { data = text ? JSON.parse(text) : {}; } catch { data = { raw: text }; }
    if (!response.ok) throw Object.assign(new Error(`Local vision error (${response.status})`), { status: 502, details: data });
    const result = String(data?.message?.content || '').trim(); if (!result) throw Object.assign(new Error('Local vision returned an empty response'), { status: 502, details: data });
    return { result, model: data.model || options.model || OLLAMA_MODEL, totalDuration: data.total_duration || null };
  } catch (error) { if (error.name === 'AbortError') throw Object.assign(new Error('Локальный AI не успел проанализировать изображение'), { status: 504 }); throw error; } finally { clearTimeout(timer); }
}
