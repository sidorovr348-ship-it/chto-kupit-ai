...
const startDeepSeek = s.indexOf('async function callDeepSeek(');
const endDeepSeek = s.indexOf('\nasync function callOpenRouter(', startDeepSeek);
if (startDeepSeek >= 0 && endDeepSeek > startDeepSeek) {
  const fn = `async function callDeepSeek(messages, images = []) {
  let key = String(process.env.DEEPSEEK_API_KEY || '').trim();
  const homoglyphs = { 'с':'c','С':'C','а':'a','А':'A','е':'e','Е':'E','о':'o','О':'O','р':'p','Р':'P','х':'x','Х':'X','у':'y','У':'Y','к':'k','К':'K','м':'m','М':'M','т':'t','Т':'T','в':'b','В':'B','н':'h','Н':'H' };
  key = [...key].map(ch => homoglyphs[ch] || ch).join('');
  if (!key) throw new Error('DEEPSEEK_API_KEY is not configured');
  if (/[\\x00-\\x1F\\x7F-\\uFFFF]/.test(key)) throw new Error('DEEPSEEK_API_KEY contains unsupported characters');
...
`;
  s = s.slice(0, startDeepSeek) + fn + s.slice(endDeepSeek);
} else if (!s.includes('async function callDeepSeek(')) {
  const marker = '\nasync function callOpenRouter(messages, images = []) {';
  if (!s.includes(marker)) throw new Error('callOpenRouter marker not found');
