...
app.post('/alice',async(req,res)=>{const c=String(req.body?.request?.command||req.body?.request?.original_utterance||'').trim();if(!c||/^ping$/iu.test(c))return res.json({version:'1.0',response:{text:'Мой AI на связи.',end_session:false}});try{const r=await ask([{role:'user',content:c}],false);res.json({version:'1.0',response:{text:clean(r).slice(0,1024),end_session:false}});}catch(e){console.error('ALICE_ERROR:',e?.stack||e);res.status(502).json({version:'1.0',response:{text:'Сейчас не удалось получить ответ. Попробуйте ещё раз.',end_session:false},error:e.message});}});
